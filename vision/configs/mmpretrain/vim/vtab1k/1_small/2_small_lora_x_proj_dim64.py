...
model = dict(
    ...
    backbone=dict(
        type='VimPEFTModel',
        lora_x_proj=True, dim_x_proj=64, s_x_proj=0.1, 
        module=dict(
            type='VisionMamba',
...
optim_wrapper = dict(
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            '_scan_addi': dict(decay_mult=1.0, lr_mult=1.0),
            'lora_B': dict(decay_mult=1., lr_mult=1.),
            'lora_C': dict(decay_mult=1.0, lr_mult=1.0),
            'lora_X': dict(decay_mult=0.5, lr_mult=5.0),
            'lora_Z': dict(decay_mult=10.0, lr_mult=1.0),
            'lora_conv1d': dict(decay_mult=1.0, lr_mult=1.0),
            'lora_d': dict(decay_mult=5.0, lr_mult=0.5),
            'lora_dt': dict(decay_mult=5.0, lr_mult=10.0),
            'lora_in_proj': dict(decay_mult=10.0, lr_mult=1.0),
            'lora_out_proj': dict(decay_mult=0.1, lr_mult=0.5),
            'lora_x_proj': dict(decay_mult=1., lr_mult=1.0),
            'prefix_encoder': dict(decay_mult=0.5, lr_mult=1.0),
            'prompt_encoder': dict(decay_mult=1.0, lr_mult=1.0)
        }),
